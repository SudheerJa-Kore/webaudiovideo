...
public class PermissionManager implements IPermissionManager
{
    ...
    {
        MainApp.getGlobalContext().checkCallingOrSelfPermission(permissionType.getTypeName());
        int permissionCheck = ContextCompat.checkSelfPermission(MainApp.getGlobalContext(), permissionType.getTypeName());
    ...
    public void requestPermission(Activity activity, List<PermissionManagerType> permissionTypeList, List<PermissionRequest> permissionRequestList)
    {
        ...
        for (int i = 0; i < permissionTypeList.size(); i++)
        {
            ...
            boolean isLastPermission = i == (permissionTypeList.size()-1);
            int permissionCheck = ContextCompat.checkSelfPermission(MainApp.getGlobalContext(), permissionManagerType.getTypeName());
    ...
    {
        try {
            PackageInfo info = MainApp.getGlobalContext().getPackageManager().getPackageInfo
                    (MainApp.getGlobalContext().getPackageName(), PackageManager.GET_PERMISSIONS);
    ...
    public static void askForSystemAlertWindowPermission() {//}), int requestCode) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            Intent intent = new Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS,
                    Uri.parse("package:" + MainApp.getGlobalContext().getPackageName()));
            ///activity.startActivityForResult(intent, requestCode);
            MainApp.getGlobalContext().startActivity(intent);
        } else {
            Log.d(TAG,"permission system alert window already enabled");
        }
    }
}